...
function taskCreated(req, res) {
	...
	const task = getTaskFromRequestBody(req.body);
	const integrationId = req.params.integrationId;

	// respond immediately
	res.sendStatus(200);
	
	onTaskCreatedOrUpdated(task, integrationId);
...
function getTaskFromRequestBody(body) {
...
function onTaskCreatedOrUpdated(taskData, integrationId) {
	const auth = Endpoints.authToken();
	getIntegrationWithId(integrationId, auth, (error, integration) => {
		if (error) {
			return log('error', 'teamwork-get-integration-response', JSON.stringify(error));
